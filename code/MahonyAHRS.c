...
{
    //返回的是弧度值 注意IFx 是 x y
    //roll_mahony = (float)Ifx_LutAtan2F32_fxpAngle(0.5f - q1*q1 - q2*q2,q0*q1 + q2*q3);

    //arm_atan2_f32(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2,&roll_mahony);
    roll_mahony=Ifx_LutAtan2F32_float32(q0*q1 + q2*q3, 0.5f - q1*q1 - q2*q2);
    roll_mahony *= 57.29578f;
    pitch_mahony =57.29578f * asinf(-2.0f * (q1*q3 - q0*q2));
    //yaw_mahony = (float )Ifx_LutAtan2F32_fxpAngle(0.5f - q2*q2 - q3*q3,q1*q2 + q0*q3);

    //arm_atan2_f32(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3,&yaw_mahony);
    yaw_mahony= Ifx_LutAtan2F32_float32(q1*q2 + q0*q3, 0.5f - q2*q2 - q3*q3);
