...
void PIT_init(void)
{
    ...
    //pit_init(CCU60_CH1, 10*1000);//2ms中断
    pit_ms_init(CCU60_CH0 , 1); //1ms
    pit_ms_init(CCU60_CH1, 10); //5ms
...
{
    interrupt_global_enable(0);                     // 开启中断 10ms
...
IFX_INTERRUPT(cc61_pit_ch0_isr,1, CCU6_1_CH0_ISR_PRIORITY)//注册陀螺仪中断在次核
{
...
IFX_INTERRUPT(uart2_rx_isr, 0, UART2_RX_INT_PRIO)
{
    ...
    usb_edgeboard_rx_handler();             // 无线模块统一回调函数
    ////上位机通讯开启
